...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const valid = await instance.checkValid(
...
router.put('/', auth, async(req,res)=>{
  try {
    ...
    const revision = parseInt(await instance.getRevision(req.user.walletAddress))
    const profile = JSON.stringify({
      ...req.body,
      revision: revision + 1,
    });
    ...
    const byte32_cid = getBytes32FromIpfsHash(cid);
    const data = instance.contract.methods
      .setProfile(req.user.walletAddress, byte32_cid, revision+1)
      .encodeABI();
    const receipt = await sendTransaction(instance.address, data, req.token);
    res.send(receipt.data);
  } catch (error) {
